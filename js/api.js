...
const Api = (() => {
  ...
  // Antes, cualquier JSON valido se trataba como "exito", incluso si el
  // propio Code.gs devolvia algo como {ok:false, error:"..."}. Por eso un
  // registro podia fallar en el backend y la app igual decia "guardado
  // correctamente". Esta funcion revisa las formas mas comunes de marcar
  // error y lanza un ApiError real cuando las encuentra.
  // SUPUESTO A VERIFICAR: ajusta los nombres de campo (ok/success/error) a
  // los que realmente devuelve tu Code.gs.
  function verificarExito(data, etiqueta) {
    if (!data || typeof data !== 'object' || Array.isArray(data)) return data;
    const marcaExito = data.ok ?? data.success ?? data.Ok ?? data.Success;
    const mensajeError = data.error || data.Error || data.mensaje || data.Mensaje;
    if (marcaExito === false || (mensajeError && marcaExito === undefined)) {
      throw new ApiError(mensajeError || `El servidor rechazó la operación "${etiqueta}".`, 'ERROR_API', data);
    }
    return data;
  }
