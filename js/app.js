...
const App = (() => {
  ...
  /**
   * Registro de empresa nueva: NEXUS no tiene un solo endpoint que haga
   * "todo junto", asi que aqui se encadenan los 3 confirmados:
   *   1) registrar_empresa       -> crea la empresa (esperamos que devuelva ID_Empresa)
   *   2) registrar_usuario       -> crea el usuario (esperamos que devuelva ID_Usuario)
   *   3) registrar_empresa_usuario -> vincula usuario con empresa
   * SUPUESTO A VERIFICAR: nombres de campo (nombreEmpresa, nit, representante)
   * y forma de la respuesta de cada paso; ajusta segun tu Code.gs real.
   */
  async function registrarEmpresaCompleta(datos) {
    const respuestaEmpresa = await Api.registrarEmpresa({
      nombreEmpresa: datos.empresa,
      nit: datos.nit,
      representante: datos.representante,
    });
    const idEmpresa = respuestaEmpresa?.ID_Empresa || respuestaEmpresa?.idEmpresa || respuestaEmpresa?.id;
    if (!idEmpresa) {
      throw new Api.ApiError('La empresa se registró pero el servidor no devolvió un ID_Empresa. Revisa la respuesta de "registrar_empresa" en tu Code.gs.', 'ERROR_API');
    }

    const respuestaUsuario = await Api.registrarUsuario({
      nombre: datos.representante,
      correo: datos.correo,
      contrasena: datos.contrasena,
    });
    const idUsuario = respuestaUsuario?.ID_Usuario || respuestaUsuario?.idUsuario || respuestaUsuario?.id;

    await Api.registrarEmpresaUsuario({
      idEmpresa,
      idUsuario,
      correo: datos.correo,
      rol: 'admin',
